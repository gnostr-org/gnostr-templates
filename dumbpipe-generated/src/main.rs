...
use dumbpipe_generated::NodeTicket;
use iroh_net::{
...
/// 32 byte node id.
///
/// Connecting to a node id is independent of its IP address. dumbpipe-generated will try
...
pub struct CommonArgs {
    ...
    ///
    /// This is an expert feature that allows dumbpipe-generated to be used to interact
...
impl CommonArgs {
    fn alpn(&self) -> anyhow::Result<Vec<u8>> {
        Ok(match &self.custom_alpn {
            ...
            None => dumbpipe_generated::ALPN.to_vec(),
        })
    }
...
async fn listen_stdio(args: ListenArgs) -> anyhow::Result<()> {
    ...
    eprintln!("Listening. To connect, use:\ndumbpipe-generated connect {}", ticket);
    if args.common.verbose > 0 {
        eprintln!("or:\ndumbpipe-generated connect {}", short);
    }
    ...
    loop {
        ...
        if !args.common.is_custom_alpn() {
            ...
            let mut buf = [0u8; dumbpipe_generated::HANDSHAKE.len()];
            r.read_exact(&mut buf).await?;
            anyhow::ensure!(buf == dumbpipe_generated::HANDSHAKE, "invalid handshake");
        }
...
async fn connect_stdio(args: ConnectArgs) -> anyhow::Result<()> {
    ...
    if !args.common.is_custom_alpn() {
        ...
        s.write_all(&dumbpipe_generated::HANDSHAKE).await?;
    }
    tracing::info!("forwarding stdin/stdout to {}", remote_node_id);
    forward_bidi(tokio::io::stdin(), tokio::io::stdout(), r, s).await?;
...
async fn connect_tcp(args: ConnectTcpArgs) -> anyhow::Result<()> {
    ...
    async fn handle_tcp_accept(
        ...
        if handshake {
            // the connecting side must write first. we don't know if there will be something
            // on stdin, so just write a handshake.
            magic_send.write_all(&dumbpipe_generated::HANDSHAKE).await?;
        }
...
async fn listen_tcp(args: ListenTcpArgs) -> anyhow::Result<()> {
    ...
    eprintln!("dumbpipe-generated connect-tcp {ticket}");
    if args.common.verbose > 0 {
        eprintln!("or:\ndumbpipe-generated connect-tcp {}", short);
    }
